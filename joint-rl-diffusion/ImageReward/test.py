...
import torch
import trainer.ppytorch.synthesis.text_to_image.ImageReward.ImageReward as ImageRewardModel
...
def test(args):
    ...
    for model_type in model_type_list:
        ...
        if model_type == "ImageReward-v1.0":
            model = ImageRewardModel.load(name=model_type, device=device, download_root=args.rm_path)
        else:
            model = ImageRewardModel.load_score(name=model_type, device=device, download_root=args.rm_path)
        ...
        with torch.no_grad():
            for item in score_sample:
                ...
                target_item = {"id": item["id"], "prompt": item["prompt"], "ranking": ranking, "rewards": rewards}
