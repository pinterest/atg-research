...
class Feature(object):
 ...
 metadata:FeatureStatsMetadata = None

@dataclass
class LwBatchV2(object):
    batch: Dict[EntityType, torch.Tensor] = None
@dataclass
class LwBatch(object):
    """A minibatch of features using to for training and evaluating LW models."""
    ...
    def replace(self, **attrs) -> LwBatch:
...
class LwFeaturizerV2:
    ...
    def triplet_collate_fn(self, pairs: pa.Table, index: pa.Table) -> LwBatch:
    ...
    def single_collate_fn(self, index: pa.Table) -> LwBatch:
        """
    ...
    def pair_collate_fn(self, pairs: pa.Table) -> LwBatch:
        # ["query_key", "cand_key", "query_entity_type", "cand_entity_type", "feat1", "feat2", "meta"]
        query_entity_type = pairs["query_entity_type"][0].as_py()
        candidate_entity_type = pairs["cand_entity_type"][0].as_py()
        all_feats, all_ids = self._extract_feats_and_ids(
